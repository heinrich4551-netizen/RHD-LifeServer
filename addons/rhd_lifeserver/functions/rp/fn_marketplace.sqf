/* Server-authoritative marketplace registry.
   Remote call: [action,args] remoteExecCall ['RHD_fnc_marketplace',2]
...
switch (_action) do {
    case 'LIST': {
        private _item = _args param [0,'']; private _amount = round (_args param [1,0]); private _unitPrice = round (_args param [2,0]);
        ...
        [['MARKET_LISTED',_id,_item,_amount,_unitPrice]] remoteExecCall ['RHD_fnc_rpResult',owner _caller]; true
    };
    case 'VIEW': {
        private _rows = [];
        ...
        [_rows] remoteExecCall ['RHD_fnc_rpResult',owner _caller]; true
    };
    case 'CANCEL': {
        private _id = _args param [0,'']; private _e = _market getOrDefault [_id,[]];
        if (_e isEqualTo [] || {(_e param [1,'']) isNotEqualTo _uid} || {(_e param [5,'']) isNotEqualTo 'OPEN'}) exitWith {false};
        _e set [5,'CANCELLED']; _market set [_id,_e]; missionNamespace setVariable ['RHD_Marketplace',_market,true];
        [['MARKET_CANCELLED',_id]] remoteExecCall ['RHD_fnc_rpResult',owner _caller]; true
    };
    default {false};
};
