/* Server-side RP phone service.
   Remote call: [action,args] remoteExecCall ['RHD_fnc_phone',2]
...
switch (_action) do {
    case 'REGISTER': {
        ...
        [['PHONE_NUMBER',_number]] remoteExecCall ['RHD_fnc_rpResult',owner _caller]; true
    };
    case 'CALL': {
        private _targetNumber = _args param [0,'']; private _targetUID = '';
        ...
        [['PHONE_CALL_SENT',_targetNumber]] remoteExecCall ['RHD_fnc_rpResult',owner _caller]; true
    };
    case 'MESSAGE': {
        private _targetNumber = _args param [0,'']; private _message = (_args param [1,'']) select [0,256]; private _targetUID = '';
        {if ((_phones getOrDefault [_x,'']) isEqualTo _targetNumber) exitWith {_targetUID = _x;};} forEach keys _phones;
        private _target = if (_targetUID isEqualTo '') then {objNull} else {allPlayers select {getPlayerUID _x isEqualTo _targetUID} param [0,objNull]};
        ...
        [['PHONE_MESSAGE_SENT',_targetNumber]] remoteExecCall ['RHD_fnc_rpResult',owner _caller]; true
    };
    default {false};
};
