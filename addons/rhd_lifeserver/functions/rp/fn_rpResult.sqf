...
if (_payload isEqualType [] && {count _payload > 0} && {(_payload select 0) isEqualType []}) then {
    ...
    if (_first isEqualType '' || {_first isEqualType 0}) then {missionNamespace setVariable ['RHD_MyLicenses',_rows]; hint format ['RHD LICENSES\n\n%1',_rows joinString '\n'];} else {
        ...
        if (_isBusiness) then {missionNamespace setVariable ['RHD_MyBusinesses',_rows]; private _lines=['RHD BUSINESSES','']; {_lines pushBack format ['%1 | Balance: $%2 | ID: %3',_x param [1,'Business'],round (_x param [2,0]),_x param [0,'']];} forEach _rows; hint (_lines joinString '\n');} else {missionNamespace setVariable ['RHD_VisibleImpounds',_rows]; private _lines=['RHD POLICE IMPOUNDS','']; {_lines pushBack format ['%1 | Fee $%2 | %3 | %4',_x param [0,''],_x param [3,0],_x param [2,''],if ((_x param [4,0]) isEqualTo 3) then {'RELEASED'} else {'IMPOUNDED'}];} forEach _rows; hint (_lines joinString '\n');};
    };
} else {
