...
- [x] Marketplace reconnect/server-restart seller escrow recovery acknowledgement
...
Government taxation now has configurable sales, business and income rates, minimum-charge controls, a server-side tax transaction function, tax audit records and a government revenue ledger. Authorized administrators can request a government tax summary. Court cases have a server-owned registry with authenticated police creation and administrator closure. The RHD phone foundation assigns persistent-in-session numbers and supports authenticated player calls and SMS.

Marketplace listings reserve seller inventory through the upstream `life_fnc_handleInv` acknowledgement, hold listings in explicit pending states, charge buyers before delivery, refund failed inventory delivery, credit sellers only after successful buyer acknowledgement, and return reserved inventory through an acknowledgement-based cancellation path. The acknowledgement routing is explicitly action-tagged for LIST_ACK, BUY_ACK and CANCEL_ACK. Persisted seller listings can be recovered after a reconnect/server restart: the server sends a one-time escrow-return instruction to the authenticated seller and waits for a RECOVERY_ACK before marking the reservation recovered. This avoids silently deleting a seller's reserved inventory when the marketplace registry survives a restart. Because the underlying Framework virtual inventory remains client-managed, these acknowledgements are an integration boundary rather than an independently verifiable server inventory ledger. A crash during an in-flight purchase remains subject to the limitations of the client-managed inventory boundary and should be checked during dedicated-server validation.
