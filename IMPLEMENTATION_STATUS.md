...
Legal and illegal resource activity maintains per-player progression with separate XP/levels. Harvesting contributes progression server-side. Periodic progression bonuses are now awarded through the server-side financial transaction adapter; the harvest result no longer instructs the client to add cash locally.
...
The RP layer uses server-side authorization against the upstream `players` table for privileged actions. Client-side rank variables are not trusted for authorization. Direct remote resource, processing, and delivery-contract entrypoints now also bind the supplied player object to the actual remote execution owner, preventing clients from submitting requests on behalf of another online player.
